...
4. **CI/CD:** Docker-Images werden ausschließlich per GitHub Actions gebaut und nach Docker Hub gepusht (kein lokaler `docker push`). GitHub-Repo: `csoscd/cronmanager-mcp`. Docker-Hub-Image: `cs1711/cronmanager-mcp`. Zwei Workflows analog zum Hauptprojekt (Referenz: `/opt/dev/cronmanager/.github/workflows/`):
